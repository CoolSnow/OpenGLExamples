...
{
    if( ! prog.compileShaderFromFile("bezcurve.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("bezcurve.fs",GLSLShader::FRAGMENT))
    {
        printf("Fragment shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("bezcurve.tes",GLSLShader::TESS_EVALUATION))
    {
        printf("Tess evaluation shader failed to compile!\n%s",
               prog.log().c_str());
        exit(1);
    }
    if( ! prog.compileShaderFromFile("bezcurve.tcs",GLSLShader::TESS_CONTROL))
    {
    ...
    if( ! solidProg.compileShaderFromFile("solid.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               solidProg.log().c_str());
        exit(1);
    }
    if( ! solidProg.compileShaderFromFile("solid.fs",GLSLShader::FRAGMENT))
    {
        printf("Fragment shader failed to compile!\n%s",
               solidProg.log().c_str());
        exit(1);
